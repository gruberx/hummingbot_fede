    ...
    def clean_executor(self):
        if self.status in [PositionExecutorStatus.CLOSED_BY_TIME_LIMIT,
            ...
            if self.take_profit_order.order and (
                ...
                self._strategy.logger().info(f"Take profit order status: {self.take_profit_order.order.status}")
...
class SignalFactory:
    ...
    def all_data_sources_ready(self):
        checks = np.array([[candles.is_ready for trading_pair, candles in trading_pairs_candles.items()]
