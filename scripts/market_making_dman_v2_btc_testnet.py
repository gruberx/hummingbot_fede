...
from controllers.dman_v2 import DManV2, DManV2Config
...
class MarketMakingDmanV2(ScriptStrategyBase):
    ...
    config_v2 = DManV2Config(
        ...
        order_levels=[
            ...
            OrderLevel(level=1, side=TradeType.BUY, order_amount_usd=Decimal(60),
                       spread_factor=Decimal(1.0), order_refresh_time=60 * 5,
                       cooldown_time=15, triple_barrier_conf=triple_barrier_conf),
            OrderLevel(level=2, side=TradeType.BUY, order_amount_usd=Decimal(80),
                       spread_factor=Decimal(2.0), order_refresh_time=60 * 5,
                       cooldown_time=15, triple_barrier_conf=triple_barrier_conf),
            OrderLevel(level=0, side=TradeType.SELL, order_amount_usd=Decimal(30),
                       spread_factor=Decimal(0.8), order_refresh_time=60 * 5,
                       cooldown_time=15, triple_barrier_conf=triple_barrier_conf),
            OrderLevel(level=1, side=TradeType.SELL, order_amount_usd=Decimal(60),
                       spread_factor=Decimal(1.0), order_refresh_time=60 * 5,
                       cooldown_time=15, triple_barrier_conf=triple_barrier_conf),
            OrderLevel(level=2, side=TradeType.SELL, order_amount_usd=Decimal(80),
                       spread_factor=Decimal(2.0), order_refresh_time=60 * 5,
                       cooldown_time=15, triple_barrier_conf=triple_barrier_conf),
        ],
        candles_config=[
            CandlesConfig(connector="binance_perpetual_testnet", trading_pair=trading_pair, interval="3m", max_records=1000),
        ],
        leverage=10,
